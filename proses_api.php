<?php
    ...
    if($postjson['aksi'] == "proses_register"){
        ...
        if($cekemail['correo'] == $postjson['correo']) {
            ...
            $pass = md5($postjson['contrasena']);
            $insert = mysqli_query($mysqli, "INSERT INTO persona(primer_apellido,segundo_apellido,primer_nombre,segundo_nombre,codigo,contrasena,correo,direccion,fecha_nacimiento,FK_id_departamento,FK_id_genero,FK_id_municipio,FK_id_tipo_documento,numero_documento,FK_id_rol,estado_cuenta,puntaje) VALUES ('$postjson[pApellido]','$postjson[sApellido]','$postjson[pNombre]','$postjson[sNombre]','$postjson[codigo]','$pass','$postjson[correo]','$postjson[direccion]','$postjson[fechaNacimiento]','$postjson[departamento]','$postjson[genero]','$postjson[municipio]','$postjson[tipDoc]','$postjson[numDoc]',1,1,0);
    ...
    //LOGIN
        ...
        echo $result;
    } else if ($postjson['aksi'] == "validate_pass") {
        $password = md5($postjson['pass']);
        $cekpassword = mysqli_fetch_array(mysqli_query($mysqli, "CALL SelectContrasena('$postjson[id]');"));
        if ($password == $cekpassword['contrasena']) {
        ...
        if ($postjson['correo'] != $postjson['auxCorreo']) {
            ...
            if ($cekemail['correo'] == $postjson['correo']) {
                ...
                if ($update) {
                    ...
                    $result = json_encode(array('success'=>false, 'msg'=>'La contraseña no coincide'));
                }
            }
        } else {
            if ($postjson['contrasena'] == "") {
        ...
        if ($cekemail['correo'] == $postjson['correo']) {
            ...
            $pass = md5($postjson['contrasena']);
            $insert = mysqli_query($mysqli, "INSERT INTO persona(primer_apellido,segundo_apellido,primer_nombre,segundo_nombre,codigo,contrasena,correo,direccion,fecha_nacimiento,FK_id_departamento,FK_id_genero,FK_id_municipio,FK_id_tipo_documento,numero_documento,FK_id_rol,estado_cuenta,puntaje) VALUES ('$postjson[pApellido]','$postjson[sApellido]','$postjson[pNombre]','$postjson[sNombre]','$postjson[codigo]','$pass','$postjson[correo]','$postjson[direccion]','$postjson[fechaNacimiento]','$postjson[departamento]','$postjson[genero]','$postjson[municipio]','$postjson[tipDoc]','$postjson[numDoc]',2,1,0);
            ");
        ...
        echo $result;
    } else if ($postjson['aksi'] == "log_error") {
        
        $insert = mysqli_query($mysqli, "CALL LogError('$postjson[codigo]','$postjson[fuente]')");
        if ($insert) {
            $result = json_encode(array('success'=>true));
        } else {
            $result = json_encode(array('success'=>false));
